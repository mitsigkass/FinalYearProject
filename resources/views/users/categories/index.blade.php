  ...
  <div class="row justify-content-center">
    <div class="col-md-3">
      <div class="card">
        ...
        <div class="card-header">
          <strong>{{$main->maincategory}} - Subcategories</strong>
        </div>
                ...
                @if(isset($subcategories))
                  @if(count($subcategories)>0)
                    @foreach($subcategories as $subcategory)
                      <li>
                        <a href='{{url('/viewsubsads/'.$main->id.'/'.preg_replace('/\s+/','',$subcategory->subcategory).'/'.$subcategory->id)}}'>{{$subcategory->subcategory}}</a>
                      </li>
                    @endforeach
                  @else
